...
var PRECOMMIT_HANDLER_SUPPORTED = new InjectionToken("", {
  factory: () => {
    return (
      ...
      typeof window.NavigationPrecommitController !== "undefined"
    );
  },
});
...
var NoTrailingSlashPathLocationStrategy = class _NoTrailingSlashPathLocationStrategy extends PathLocationStrategy {
  ...
  static ɵfac = ɵɵngDeclareFactory({
    ...
    version: "21.2.12",
    ngImport: core_exports,
    type: _NoTrailingSlashPathLocationStrategy,
    deps: null,
    target: FactoryTarget.Injectable,
  });
  static ɵprov = ɵɵngDeclareInjectable({
...
var TrailingSlashPathLocationStrategy = class _TrailingSlashPathLocationStrategy extends PathLocationStrategy {
  ...
  static ɵfac = ɵɵngDeclareFactory({
    ...
    version: "21.2.12",
    ngImport: core_exports,
    type: _TrailingSlashPathLocationStrategy,
    deps: null,
    target: FactoryTarget.Injectable,
  });
  static ɵprov = ɵɵngDeclareInjectable({
...
function extractUrlPath(url) {
  ...
  const pathEnd =
    questionMarkOrHashIndex > -1 ? questionMarkOrHashIndex : url.length;
...
function warnIfSignal(pipeName, value) {
  if (isSignal(value)) {
    console.warn(
      `The ${pipeName} does not unwrap signals. Received a signal with value:`,
      value(),
    );
  }
}
...
function createCloudflareUrl(path, config) {
  ...
  if (config.loaderParams?.["transform"]) {
    const transformStr = normalizeLoaderTransform(
      config.loaderParams["transform"],
      "=",
    );
    params += `,${transformStr}`;
  }
...
function createImgixUrl(path, config) {
  ...
  if (config.loaderParams?.["transform"]) {
    const transform = normalizeLoaderTransform(
      config.loaderParams["transform"],
      "=",
    ).split(",");
...
var MOCK_PLATFORM_LOCATION_CONFIG = new InjectionToken(
...
var MockPlatformLocation = class _MockPlatformLocation {
  ...
  replaceState(state, title, newUrl) {
    ...
    this.urlChanges[this.urlChangeIndex] = __spreadProps(
      __spreadValues({}, this.urlChanges[this.urlChangeIndex]),
      {
        pathname,
        search,
        hash,
  ...
  pushState(state, title, newUrl) {
    ...
    this.urlChanges.push(
      __spreadProps(__spreadValues({}, this.urlChanges[this.urlChangeIndex]), {
        pathname,
        search,
        hash,
        state: parsedState,
...
var FakeNavigationPlatformLocation = class _FakeNavigationPlatformLocation {
  ...
  constructor() {
    ...
    if (!(platformNavigation instanceof FakeNavigation)) {
      throw new Error(
        "FakePlatformNavigation cannot be used without FakeNavigation. Use `provideFakeNavigation` to have all these services provided together.",
      );
    }
  ...
  onPopState(fn) {
    ...
    return () =>
      this._platformNavigation.window?.removeEventListener?.("popstate", fn);
  }
  onHashChange(fn) {
    this._platformNavigation.window?.addEventListener?.("hashchange", fn);
    return () =>
      this._platformNavigation.window?.removeEventListener?.("hashchange", fn);
  }
  get href() {
...
var platformBrowserTesting = createPlatformFactory(
  platformBrowser,
  "browserTesting",
);
...
_TestModule_decorators = [
  NgModule({ providers: [provideZoneChangeDetection()] }),
];
var TestModule = class {};
_init = __decoratorStart(null);
TestModule = __decorateElement(
  _init,
  0,
  "TestModule",
  _TestModule_decorators,
