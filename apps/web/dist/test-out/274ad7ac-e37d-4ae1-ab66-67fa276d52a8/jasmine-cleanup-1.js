...
function patchJasmine(Zone2) {
  Zone2.__load_patch("jasmine", (global3, Zone3, api) => {
    ...
    const disablePatchingJasmineClock =
      global3[symbol("fakeAsyncDisablePatchingClock")] === true;
    const enableAutoFakeAsyncWhenClockPatched =
      !disablePatchingJasmineClock &&
      (global3[symbol("fakeAsyncPatchLock")] === true ||
        global3[symbol("fakeAsyncAutoFakeAsyncWhenClockPatched")] === true);
    ...
    if (!disablePatchingJasmineClock) {
      ...
      jasmine["clock"] = function () {
        ...
        if (!clock[symbol("patched")]) {
          ...
          clock.tick = function () {
            const fakeAsyncZoneSpec = Zone3.current.get(
          ...
          if (enableAutoFakeAsyncWhenClockPatched) {
            ["install", "uninstall"].forEach((methodName) => {
              const originalClockFn2 = (clock[symbol(methodName)] =
                clock[methodName]);
    ...
    function wrapDescribeInZone(description, describeBody) {
      return function () {
        const syncZone = ambientZone.fork(
          new SyncTestZoneSpec(`jasmine.describe#${description}`),
        );
    ...
    function runInTestZone(testBody, applyThis, queueRunner, done) {
      ...
      if (isClockInstalled && enableAutoFakeAsyncWhenClockPatched) {
        ...
        if (
          fakeAsyncModule &&
          typeof fakeAsyncModule.fakeAsync === "function"
        ) {
    ...
    const j$ = jasmine;
    const privateApis = (
      (_a = j$ == null ? void 0 : j$.private) == null ? void 0 : _a.QueueRunner
    )
      ? j$ == null
        ? void 0
        : j$.private
    ...
    privateApis.QueueRunner = (function (_super) {
      ...
      function ZoneQueueRunner(attrs) {
        ...
        attrs.onException = function (error) {
          if (
            ...
            if (proxyZoneSpec) {
              const pendingTasksInfo =
                proxyZoneSpec.getAndClearPendingTasksInfo();
              try {
      ...
      ZoneQueueRunner.prototype.execute = function () {
        ...
        if (!Zone3.currentTask) {
          Zone3.current.scheduleMicroTask("jasmine.execute().forceTask", () =>
...
function patchMocha(Zone2) {
  Zone2.__load_patch("mocha", (global3, Zone3) => {
    ...
    global3.xdescribe =
      global3.suite.skip =
      global3.describe.skip =
        function () {
          return mochaOriginal.describe.skip.apply(
            this,
            wrapDescribeInZone(arguments),
          );
        };
    global3.describe.only = global3.suite.only = function () {
      return mochaOriginal.describe.only.apply(
        this,
        wrapDescribeInZone(arguments),
      );
    };
...
var AsyncTestZoneSpec = class _AsyncTestZoneSpec {
    ...
    if (
      ...
      this.runZone.run(() => {
        this._existingFinishTimer = setTimeout(() => {
          if (
            !this._alreadyErrored &&
            !(this._pendingMicroTasks || this._pendingMacroTasks)
          ) {
            this.finishCallback();
          }
        }, 0);
  ...
  unPatchPromiseForTest() {
    ...
    const unPatchPromiseForTest =
      Promise[Zone.__symbol__("unPatchPromiseForTest")];
  ...
  Zone2.__load_patch("asynctest", (global3, Zone3, api) => {
    ...
    function runInTestZone(
      ...
      if (AsyncTestZoneSpec2 === void 0) {
        throw new Error(
          "AsyncTestZoneSpec is needed for the async() test helper but could not be found. Please make sure that your environment includes zone.js/plugins/async-test",
        );
      }
      const ProxyZoneSpec2 = Zone3["ProxyZoneSpec"];
      if (!ProxyZoneSpec2) {
        throw new Error(
          "ProxyZoneSpec is needed for the async() test helper but could not be found. Please make sure that your environment includes zone.js/plugins/proxy",
        );
      }
...
var ProxyZoneSpec = class _ProxyZoneSpec {
  ...
  setDelegate(delegateSpec) {
    ...
    this.propertyKeys &&
      this.propertyKeys.forEach((key) => delete this.properties[key]);
    this.propertyKeys = null;
    if (delegateSpec && delegateSpec.properties) {
      this.propertyKeys = Object.keys(delegateSpec.properties);
      this.propertyKeys.forEach(
        (k) => (this.properties[k] = delegateSpec.properties[k]),
      );
    }
    if (
      isNewDelegate &&
  ...
  onFork(parentZoneDelegate, currentZone, targetZone, zoneSpec) {
    if (this._delegateSpec && this._delegateSpec.onFork) {
      return this._delegateSpec.onFork(
        parentZoneDelegate,
        currentZone,
        targetZone,
        zoneSpec,
  ...
  onHandleError(parentZoneDelegate, currentZone, targetZone, error) {
    if (this._delegateSpec && this._delegateSpec.onHandleError) {
      return this._delegateSpec.onHandleError(
        parentZoneDelegate,
        currentZone,
        targetZone,
        error,
  ...
  onScheduleTask(parentZoneDelegate, currentZone, targetZone, task) {
    ...
    if (this._delegateSpec && this._delegateSpec.onScheduleTask) {
      return this._delegateSpec.onScheduleTask(
        parentZoneDelegate,
        currentZone,
        targetZone,
        task,
  ...
  onCancelTask(parentZoneDelegate, currentZone, targetZone, task) {
    ...
    if (this._delegateSpec && this._delegateSpec.onCancelTask) {
      return this._delegateSpec.onCancelTask(
        parentZoneDelegate,
        currentZone,
        targetZone,
        task,
...
var _Scheduler = class _Scheduler2 {
  ...
  tick(millis = 0, doTick, tickOptions) {
    ...
    while (schedulerQueue.length > 0) {
      ...
      if (finalTime < current.endTime) {
        ...
        let retval = current2.func.apply(
          global2,
          current2.isRequestAnimationFrame
            ? [this._currentTickTime]
            : current2.args,
        );
        if (!retval) {
          break;
  ...
  flushNonPeriodic(limit, doTick) {
    ...
    while (this._schedulerQueue.length > 0) {
      ...
      if (
        this._schedulerQueue.filter(
          (task) => !task.isPeriodic && !task.isRequestAnimationFrame,
        ).length === 0
      ) {
        break;
      }
...
var FakeAsyncTestZoneSpec = class _FakeAsyncTestZoneSpec {
  constructor(
    ...
    if (!this.macroTaskOptions) {
      this.macroTaskOptions =
        global2[Zone.__symbol__("FakeAsyncTestMacroTask")];
    }
  }
  static assertInZone() {
      ...
      case "microTask":
        ...
        if (args) {
          ...
          if (
            ...
            args.length > callbackIndex + 1
          ) {
            additionalArgs = Array.prototype.slice.call(
              args,
              callbackIndex + 1,
            );
          }
        ...
        switch (task.source) {
          case "setTimeout":
            task.data["handleId"] = this._setTimeout(
              task.invoke,
              task.data["delay"],
              Array.prototype.slice.call(task.data["args"], 2),
            );
            break;
          case "setImmediate":
            task.data["handleId"] = this._setTimeout(
              task.invoke,
              0,
              Array.prototype.slice.call(task.data["args"], 1),
            );
            break;
          case "setInterval":
          ...
          default:
            ...
            throw new Error(
              "Unknown macroTask scheduled in fake async test: " + task.source,
            );
        }
        break;
      case "eventTask":
...
function resetFakeAsyncZone() {
  ...
  (_b = (_a = getProxyZoneSpec()) == null ? void 0 : _a.get()) == null
    ? void 0
    : _b.resetDelegate();
    ...
    try {
      ...
      if (!flush2) {
        if (_fakeAsyncTestZoneSpec.pendingPeriodicTimers.length > 0) {
          throw new Error(
            `${_fakeAsyncTestZoneSpec.pendingPeriodicTimers.length} periodic timer(s) still in the queue.`,
          );
        }
...
function withProxyZone(fn) {
  const autoProxyFn = function (...args) {
    ...
    const proxyZone =
      proxyZoneSpec.get() !== void 0 ? Zone.current : getOrCreateRootProxy();
...
function getOrCreateRootProxy() {
  ...
  if (ProxyZoneSpec2 === void 0) {
    throw new Error(
      "ProxyZoneSpec is needed for withProxyZone but could not be found. Make sure that your environment includes zone-testing.js",
    );
  }
    ...
    onScheduleTask: function (
      ...
      if (stackTracesEnabled()) {
        ...
        let trace =
          (currentTask &&
            currentTask.data &&
            currentTask.data[creationTrace]) ||
          [];
  ...
  class SyncTestZoneSpec {
    ...
    onScheduleTask(delegate, current, target, task) {
      switch (task.type) {
        ...
        case "macroTask":
          throw new Error(
            `Cannot call ${task.source} from within a sync test (${this.name}).`,
          );
        case "eventTask":
          task = delegate.scheduleTask(target, task);
          break;
...
function patchPromiseTesting(Zone2) {
  Zone2.__load_patch("promisefortest", (global3, Zone3, api) => {
    ...
    Promise[api.symbol("patchPromiseForTest")] =
      function patchPromiseForTest() {
        let oriThen = Promise[Zone3.__symbol__("ZonePromiseThen")];
        if (oriThen) {
          return;
        }
        oriThen = Promise[Zone3.__symbol__("ZonePromiseThen")] =
    ...
    Promise[api.symbol("unPatchPromiseForTest")] =
      function unpatchPromiseForTest() {
