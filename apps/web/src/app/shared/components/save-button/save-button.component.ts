...
@Component({
  ...
  template: `
    <button
      ...
      (click)="onClick()"
      class="relative flex items-center justify-center gap-2 px-6 py-3 font-sans font-semibold tracking-wide text-white transition-all duration-300 ease-out cursor-pointer select-none active:scale-[0.98] focus:outline-hidden focus:ring-2 focus:ring-primary-300 focus:ring-offset-2 disabled:cursor-not-allowed disabled:active:scale-100 disabled:opacity-60"
      [ngClass]="{
        'bg-primary hover:bg-primary-700 shadow-brand hover:shadow-brand-lg': !isLoading() && !isSuccess() && !disabled(),
        'bg-success hover:bg-success shadow-md shadow-success/20': isSuccess() && !disabled(),
    ...
    >
      ...
      <span 
        ...
        [ngClass]="{ 'w-5 opacity-100': isLoading(), 'w-0 opacity-0 overflow-hidden': !isLoading() }"
      >
      ...
      <span 
        class="flex items-center justify-center transition-all duration-300 ease-out"
        [ngClass]="{ 'w-5 opacity-100 scale-100': !isLoading() && !isSuccess(), 'w-0 opacity-0 scale-50 overflow-hidden': isLoading() || isSuccess() }"
      >
      ...
      <span 
        class="flex items-center justify-center transition-all duration-300 ease-out"
        [ngClass]="{ 'w-5 opacity-100 scale-100': isSuccess() && !isLoading(), 'w-0 opacity-0 scale-50 overflow-hidden': !isSuccess() || isLoading() }"
      >
      ...
      <span class="absolute inset-0 w-full h-full bg-white/5 opacity-0 hover:opacity-100 transition-opacity duration-300 rounded-brand pointer-events-none"></span>
    </button>
  `,
  styles: [`
    :host {
